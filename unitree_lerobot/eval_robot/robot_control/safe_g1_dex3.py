...
class _G1Dex3CommandBackend:
    ...
    def _validate_runtime_state(self, state: RobotState) -> RobotState:
        ...
        if self._uses_mode5_waist_hold():
            # Match the established XR teleoperate behavior: validate that
            # mode-5 waist feedback is structurally usable, but do not fault
            # on balance/locomotion motion while arm_sdk remains active.
            self._require_mode5_waist_state(state)
